...
def slide_list(input_dir):
    """
    Create list of paths of slides to be preprocessed
    """
    csv = pd.read_csv('/home/jm2239/SCAN_MASTER_DEID_6_1_2020.csv')
    label_list = ["OLIGO","IA_IDH_WT","IA_IDH_MUTANT"]
    glioma_list = []
    for label in label_list:
        ...
        print(label)
        print(label_list)
        glioma_list.extend(label_list)
    glioma_paths = []
    for file_name in os.listdir(input_dir):
        if (file_name[-4:] == '.svs'):
            if file_name[:-4] in glioma_list:
                glioma_paths.append(input_dir+file_name)
    return glioma_paths
...
def main():
    glioma_paths = slide_list("/nfs03/data/weill_pathology/cajalnet_data/")
    slides = glioma_paths
    #nonglioma = label with 0
    targets = [1]*len(slides)
    mult = [1.]*len(slides)
    print("The number of processing slidepaths: ", len(slides))
